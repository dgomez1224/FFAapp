...
export type RivalryLevel = "intense" | "moderate" | "friendly";
...
export function rivalryChipLabel(type?: string | null) {
  switch (type) {
    case "french_derby":
      return "French Derby";
    case "dalton_derby":
    ...
    case "cugino_derby":
      return "Cugino Derby";
    case "lansing_derby":
      return "Lansing Derby";
    case "sibling_derby":
      return "Sibling Derby";
    case "misc":
      return "Rivalry";
    default:
      return type || "rivalry";
...
export function findRivalry(managerA: string, managerB: string): Rivalry | null {
  const a = normalizeManagerName(managerA);
  const b = normalizeManagerName(managerB);
...
function h2hSentence(a: string, b: string, h2h?: RivalryH2H | null) {
  if (!h2h) return "";
  const played = h2h.aWins + h2h.bWins + h2h.draws;
  if (played <= 0) return "";
  let line = `All-time H2H: ${a} ${h2h.aWins}–${h2h.draws}–${h2h.bWins} ${b} (${played} meetings).`;
  if (h2h.aWins > h2h.bWins) line += ` ${a} holds the historical edge.`;
  else if (h2h.bWins > h2h.aWins) line += ` ${b} holds the historical edge.`;
  else line += " This rivalry has been evenly matched.";
  return line;
}

function contextLine(rivalry: Rivalry) {
  return rivalry.background || rivalry.description;
}
...
export function buildRivalryNewsItem(opts: {
  ...
  scoreB?: number;
  h2h?: RivalryH2H | null;
  topPerformers?: RivalryPerformer[];
  publishedAt: string;
  matchupUrl?: string;
}): RivalryNewsDraft {
  ...
  const h2hLine = h2hSentence(a, b, opts.h2h);
  const flavor = contextLine(rivalry);
